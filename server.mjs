...
const runCnnOcrVision = async ({ imageData, language, barcode, qrContent, provider }) => {
  ...
  const prompt = localizedPrompt(language) + suppliedCodes + '\nThis is the CNN + OCR + AI vision pass. Keep the visionPipeline fields evidence-based and concise.'
  const report = provider === 'gemini' ? await analyzeWithGemini(imageData, prompt) : await analyzeWithNvidia(imageData, prompt)
  return normalizeReport(report, { barcode, qrContent })
}
