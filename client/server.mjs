...
const analyzeWithNvidia = async (imageData, prompt) => {
  const upstream = await fetch('https://integrate.api.nvidia.com/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: nvidiaModel, temperature: 0.1, max_tokens: 1800, messages: [{ role: 'user', content: [{ type: 'text', text: `${prompt}\nReturn one JSON object only. Do not use markdown fences or explanatory text.` }, { type: 'image_url', image_url: { url: `data:${imageData.type};base64,${imageData.base64}` } }] }] }),
  })
  const payload = await upstream.json()
