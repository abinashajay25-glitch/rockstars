...
AI packaged commodity compliance checker with camera/image intake, a hybrid CNN-style visual pass, OCR + barcode/QR evidence, AI vision inspection, seven-point label validation, health and technology intelligence, market comparison, multilingual reports, PDF printing, and ElevenLabs voice output.

Each inspection runs through `runCnnOcrVision`: the model is instructed to first assess package regions and legibility (CNN-style visual features), then extract readable text (OCR), and finally reconcile that evidence with barcode/QR data before producing the compliance report. Unclear evidence remains `REVIEW` or `UNKNOWN` rather than being invented.
