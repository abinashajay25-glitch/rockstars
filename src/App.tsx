...
function App() {
  ...
  const downloadPdf = async () => {
    ...
    const margin = 15
    const width = 180
    const lines = (value: string) => pdf.splitTextToSize(value, width)
    const writeSection = (title: string, body: string, startY: number) => {
      pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11); pdf.setTextColor(10, 10, 9); pdf.text(title, margin, startY)
      pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8.5)
      const bodyLines = lines(body); pdf.text(bodyLines, margin, startY + 5)
    ...
    y = 32
    y = writeSection('EXTRACTED PACKAGE INFORMATION', packageInfo || 'No package fields returned.', y)
    y = writeSection('COMPLIANCE', `Score: ${analysis.complianceScore}% / ${analysis.complianceStatus}\n${compliance}`, y + 4)
