...
function App() {
  ...
  const downloadPdf = async () => {
    ...
    const writeSection = (title: string, body: string, startY: number, x = margin, sectionWidth = width) => {
      pdf.setFont('helvetica', 'bold'); pdf.setFontSize(11); pdf.setTextColor(10, 10, 9); pdf.text(title, x, startY)
    ...
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(18); pdf.setTextColor(10, 10, 9); pdf.text(`${text.report} / 01`, margin, 20)
    pdf.setFont('helvetica', 'normal'); pdf.setFontSize(9); pdf.text(lines(`Inspection date: ${date}\n${analysis.productName} / ${analysis.brand} / ${analysis.category}\n\n${analysis.summary}`), margin, 30)
    if (file) { try { pdf.addImage(await imageDataUrl(file), 'JPEG', margin, 62, 180, 72, undefined, 'MEDIUM') } catch { /* Keep the text report if the browser cannot encode the image. */ } }
    let y = 145
    ...
    pdf.addPage()
    pdf.setFont('helvetica', 'bold'); pdf.setFontSize(18); pdf.setTextColor(10, 10, 9); pdf.text(`${text.report} / 02`, margin, 20)
    y = 32
    let leftY = 32
    leftY = writeSection('EXTRACTED PACKAGE INFORMATION', packageInfo || 'No package fields returned.', leftY, 15, 85)
    writeSection('COMPLIANCE', `Score: ${analysis.complianceScore}% / ${analysis.complianceStatus}\n${compliance}`, leftY + 4, 15, 85)
    let rightY = 32
