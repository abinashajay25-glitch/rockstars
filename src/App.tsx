...
function App() {
  ...
  const [isBarcodeUploadOpen, setIsBarcodeUploadOpen] = useState(false)
  ...
  const decodePackageCode = async (url: string, isSeparateBarcodeImage = false) => {
    ...
    try {
      ...
      setDecodedCode(null)
      setCodeMessage(isSeparateBarcodeImage ? 'Barcode not readable — please upload a clearer image.' : 'Scan Barcode / Upload Barcode Image')
      return null
    }
  }
  ...
  const selectBarcodeFile = (event: ChangeEvent<HTMLInputElement>) => {
    const selected = event.target.files?.[0]
    if (!selected || !selected.type.startsWith('image/')) { setCodeMessage('Barcode not readable — please upload a clearer image.'); return }
    const url = URL.createObjectURL(selected)
    void decodePackageCode(url, true).finally(() => URL.revokeObjectURL(url))
    event.target.value = ''
  }
  ...
  const downloadPdf = async () => {
    ...
    y = 31
    drawSection(labels.health, [`${labels.edibility}: ${value(analysis.health.edibility, 'NEEDS_REVIEW')}`, `${labels.ingredients}: ${value(analysis.health.ingredients)}`, `${labels.nutriScore}: ${value(analysis.health.nutriScore)}`, `${labels.allergens}: ${value(analysis.health.allergens)}`, `${labels.additives}: ${value(analysis.health.additives)}`].join('\n'), margin, y, columnWidth, 45)
