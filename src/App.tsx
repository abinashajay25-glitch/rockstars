...
type Analysis = {
  provider?: string; cnnCategory?: string; productName: string; category: string; brand: string; summary: string
...
function App() {
  ...
  const analyzeImage = async () => {
    ...
    try {
      /* Step 1: CNN Detect — lightweight category hint for the vision model */
      await delay(900)
  ...
  const downloadPdf = async () => {
    ...
    const ensureSpace = (need: number) => { if (y + need > 285) y = Math.max(y, 285 - need) }
